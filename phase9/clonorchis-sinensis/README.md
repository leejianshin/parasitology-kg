...
P9-B1已获单独授权并完成第三修订原子的本地实现，当前状态为
...
冻结为公开回归集，不冒充held-out证据。独立复审方另行冻结了8条未公开查询，
当前施工侧只持有套件ID、题数、冻结时间与内容SHA256；内容须在本地提交形成后
才可披露和执行。
...
`p9b1-local-acceptance.yml`。8条真正盲测尚待提交后的独立只读复审；当前不得
